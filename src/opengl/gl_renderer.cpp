...
void GLRenderer::compile_shader() { // NOTE: must run after the GL context is initialized
...
void GLRenderer::set_scene(shared_ptr<const Scene> scene) { // Set the scene and buffer each model
...
void GLRenderer::add_buffered_mesh(const Mesh& mesh, const Mat4& trans) {
  ...
  glBufferData(
    GL_ARRAY_BUFFER, sizeof(GLfloat) * mesh_prop.size(), &(mesh_prop[0]), GL_STATIC_DRAW);
  ...
  glVertexAttribPointer(
    1, 3, GL_FLOAT, GL_FALSE, stride * sizeof(GLfloat), (GLvoid*)(4 * sizeof(GLfloat)));
  glEnableVertexAttribArray(1);
  // color = location 2
  glVertexAttribPointer(
    2, 4, GL_FLOAT, GL_FALSE, stride * sizeof(GLfloat), (GLvoid*)(7 * sizeof(GLfloat)));
...
void GLRenderer::render() {
  ...
  Light dir_light {
    // static global directional light
  ...
  glBufferData(GL_UNIFORM_BUFFER, sizeof(ubPerFrame), &ubpf_data, GL_STATIC_DRAW);
...
void GLRenderer::render_meshes() {
  ...
  for (auto& buffered_mesh : meshes) {
    ...
    // Draw them
    glDrawElements(GL_TRIANGLES, (GLuint)buffered_mesh.indices_num(), GL_UNSIGNED_INT, nullptr);
  }
}
