...
int main() {
  ...
  GLFWwindow* window = glfwCreateWindow(720, 480, "LearnOpenGL", nullptr, nullptr);
  if (window == nullptr) {
  ...
  glTexImage2D(GL_TEXTURE_2D,  // texture target on the object; will not affect _1D, etc
  ...
  glVertexAttribPointer(
    1, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), (GLvoid*)(2 * sizeof(GLfloat)));
  glEnableVertexAttribArray(1);

  // Element buffer
  GLuint indices[] = {0, 1, 2, // starts from 0 !
    0, 2, 3};
  ...
  glShaderSource(vertexShader, // shader object ID
